...
class PurchaseApply(models.Model):
    ...
    warehouse_id = fields.Many2one('stock.warehouse', '入库仓库', required=1, readonly=1, states=READONLY_STATES, track_visibility='onchange', domain="[('company_id', '=', company_id)]")
    ...
    def action_confirm(self):
        ...
        if any([float_is_zero(line.product_qty, precision_rounding=0.01) for line in self.line_ids]):
    ...
    def action_update_price(self):
        ...
        for line in self.line_ids:
            ...
            supplierinfo = supplierinfo_obj.search(
                [('min_qty', '<=', line.product_qty), ('product_id', '=', line.product_id.id), ('state', '=', 'done'), '&',
    ...
    def _compute_amount(self):
        """计算预计成本"""
        for res in self:
            res.amount = sum(res.line_ids.mapped('amount'))
    ...
    def create_purchase_orders(self):
        ...
        contract_obj = self.env['supplier.contract']  # 供应商合同
        ...
        for partner, ls in groupby(sorted(self.line_ids, key=lambda x: x.supplierinfo_id.name), lambda x: x.supplierinfo_id.name):
            ...
            if contract:
                ...
                raise ValidationError('供应商：%s当前没有有效合同！' % partner.name)
            ...
            for line in ls:
                ...
                order_line_obj.create({
                    ...
                    'product_id': line.product_id.id,
                    'price_unit': line.price if payment_term.type not in ['joint'] else 0,  # 联营单价为0

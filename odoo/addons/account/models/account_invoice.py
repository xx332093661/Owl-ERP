...
class AccountInvoice(models.Model):
    ...
    def _onchange_partner_id(self):
        ...
        if p:
            ...
            rec_account = p.property_account_receivable_id  # 应收
            if not rec_account:
                receivable_id = config_obj.get_param('account.partner_account_receivable_id')
                if receivable_id:
                    domain1 = [('code', '=', account_obj.browse(int(receivable_id)).code)]
                    domain1 = expression.AND([domain1, [('company_id', '=', company_id)]])
                    rec_account = account_obj.search(domain1)

            pay_account = p.property_account_payable_id
            if not pay_account:
                payable_id = config_obj.get_param('account.partner_account_payable_id')
                if payable_id:
                    domain1 = [('code', '=', account_obj.browse(int(payable_id)).code)]
                    domain1 = expression.AND([domain1, [('company_id', '=', company_id)]])
                    pay_account = account_obj.search(domain1)
